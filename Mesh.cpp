...
Mesh::Mesh(Vertex* vertices,  int vCount, unsigned int* indices,  int iCount) 
{
	...
	Mesh::CreateVertexBuffer(vertices, vCount);
	Mesh::CreateIndexBuffer(indices, iCount);
}
...
void Mesh::CreateVertexBuffer(Vertex* vertices,  int vCount) 
{
	...
	vbd.ByteWidth = sizeof(Vertex) * vCount;       // Depends on number of vertices
...
void Mesh::CreateIndexBuffer(unsigned int* indices,  int iCount) 
{
...
void Mesh::Draw() 
{
	UINT stride = sizeof(Vertex); // Stride -> How big is one vertex in memory
	UINT offset = 0;
